    ...
    def action_create_draft_po(self):
        for order in self:
            ...
            if order.nox_product1:
                vendor_price = order.validate_product_for_vendor(order.nox_product1)
            ...
            if order.nox_product2:
                vendor_price = order.validate_product_for_vendor(order.nox_product2)
                if vendor_price != order.nox_cost_hourly_rate2:
                    raise ValidationError(_('Unit price in PO Order line cannot be different from Cost hourly rate in SO!\n\nProduct: %s'%(order.nox_product2.name)))
